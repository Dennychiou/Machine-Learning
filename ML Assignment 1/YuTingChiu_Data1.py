...
dia = datasets.fetch_openml(data_id=1460)

def BestROCcurve():
  dtc = DecisionTreeClassifier(criterion="entropy", min_samples_leaf=30)
  y_scores = model_selection.cross_val_predict(dtc, dia.data, dia.target, method="predict_proba", cv=10)
  fpr, tpr, th = roc_curve(dia.target, y_scores[:,1],pos_label="2")
...
def AUCvalue_graph():
  X = dia.data
  y = dia.target
  min_samples_leaf_values = [5, 30, 80, 500, 1000, 3000, 5000]
  auc_scores = {}
